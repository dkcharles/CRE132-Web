...
// Everything a fresh round needs, in one place, so the title screen always shows a new snake.
void Reset()
{
...
void Step()
{
    ...
    // The edge of the board is a wall now, and so is every cell the snake is already on.
    ...
    {
        // Insert(0, ...) is Add with a place to put it: the new head goes on the front.
...
void Draw()
{
    ...
    switch (state)
    {
        case State.Title:
            DrawBoard();
            Screen.Text(120, 160, "SNAKE - SPACE TO PLAY", Colour.White);
            if (Keys.WasPressed(Key.Space)) state = State.Playing;
            break;
        case State.Playing:
            // An arrow only chooses a direction, and a turn back on yourself is not one: half the
            // snake is already in the way, so the game ignores it.
        ...
        case State.GameOver:
            DrawBoard();
            Screen.Text(190, 140, "GAME OVER", Colour.White);
            Screen.Text(130, 180, "ENTER FOR TITLE", Colour.White);
